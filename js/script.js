//CV PAGE
// Smooth scroll for CV page (optional)
...
function renderWritings(filter = "all") {
    ...
    filtered.forEach(item => {
        ...
        const tagLabel = item.type === "award" ? "Award-Winning" : (tagLabels[item.category] || "Work");
...
// Show button only on pages that require scrolling
const scrollPages = ["research.html", "cv.html", "data-analytics.html"]; 
const currentPage = window.location.pathname.split("/").pop();

if (scrollPages.includes(currentPage)) {
  window.addEventListener("scroll", () => {
